...
# Ensure the parent directory is in sys.path so we can import from the root
...
# We assume the server is run from the root directory 'd:\Nikul\Project\Crawler'
...
async def _async_crawl_task(task_id: str, req: CrawlRequest):
    try:
        ...
        # Generate outputs
        output_dir = os.path.join("./output", report.domain)
